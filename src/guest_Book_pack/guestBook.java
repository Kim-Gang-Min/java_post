...
class postFrame extends JFrame{
	...
	private BufferedReader postR;
	private BufferedWriter postW;
	private String today;
	private Vector<BufferedReader> dateR= new Vector<BufferedReader>();//?
	private Vector<BufferedWriter> dateW= new Vector<BufferedWriter>();//?
	private JScrollPane chatScroll,insertScroll;
	private JTextArea insertPost;
	private JPanel insertP;
	private ImageIcon emoticon,logo;
	private JLabel emoLabel,temperature,logoLabel;
	private Image img,logoImg;
	private JButton agree, changeE,dateChoice,myInfo,exit,idontknow;
	private JTextField tempInsert;
	...
	postFrame(String str){
		...
		chatScroll.setBounds(25, 300, 850, 600);
		add(chatScroll);
		
		emoticon = new ImageIcon("./Saves/emoticon/smile.png");
		img = emoticon.getImage();
		img = img.getScaledInstance(135,155,Image.SCALE_SMOOTH);
		emoticon = new ImageIcon(img);
		
		emoLabel = new JLabel(emoticon);
		emoLabel.setBounds(35, 110, 135, 155);
		add(emoLabel);
		
		agree = new JButton("확인");
		agree.setBounds(750, 220, 115, 40);
		agree.setFont(new Font("Serif",Font.BOLD,11));
		agree.setFont(agree.getFont().deriveFont(16.0f));
		agree.setForeground(Color.WHITE);
		agree.setBackground(Color.DARK_GRAY);
		//loginB.addActionListener(logA);
		add(agree);
		
		changeE = new JButton("표정 변경");
		changeE.setBounds(750, 170, 115, 40);
		changeE.setFont(new Font("Serif",Font.BOLD,11));
		changeE.setFont(changeE.getFont().deriveFont(16.0f));
		changeE.setForeground(Color.WHITE);
		changeE.setBackground(Color.DARK_GRAY);
		//loginB.addActionListener(logA);
		add(changeE);
		
		temperature = new JLabel("체온 입력");
		temperature.setBounds(770,100,115,40);
		temperature.setFont(new Font("Serif",Font.BOLD,11));
		temperature.setFont(temperature.getFont().deriveFont(16.0f));
		add(temperature);
		
		tempInsert = new JTextField();
		tempInsert.setBounds(750,135,115,30);
		tempInsert.setFont(new Font("Serif",Font.BOLD,11));
		tempInsert.setFont(temperature.getFont().deriveFont(14.0f));
		add(tempInsert);
		
		insertPost = new JTextArea();
		insertPost.setBounds(180,110,560,155);
		insertPost.setFont(new Font("Serif",Font.BOLD,11));
		insertPost.setFont(insertPost.getFont().deriveFont(20.0f));
		
		insertScroll = new JScrollPane(insertPost);
		insertScroll.setBounds(180,110,560,155);
		add(insertScroll);
		
		insertP = new JPanel();
		insertP.setBounds(25,100,850,175);
		insertP.setBackground(Color.LIGHT_GRAY);
		add(insertP);
		
		myInfo = new JButton("내 정보 보기");
		myInfo.setBounds(50, 25, 130, 50);
		myInfo.setFont(new Font("Serif",Font.BOLD,11));
		myInfo.setFont(myInfo.getFont().deriveFont(16.0f));
		myInfo.setForeground(Color.BLACK);
		myInfo.setBackground(Color.LIGHT_GRAY);
		add(myInfo);
		
		dateChoice = new JButton("날짜 변경");
		dateChoice.setBounds(230, 25, 130, 50);
		dateChoice.setFont(new Font("Serif",Font.BOLD,11));
		dateChoice.setFont(myInfo.getFont().deriveFont(16.0f));
		dateChoice.setForeground(Color.BLACK);
		dateChoice.setBackground(Color.LIGHT_GRAY);
		add(dateChoice);
		
		
		exit = new JButton("종료 하기");
		exit.setBounds(720, 25, 130, 50);
		exit.setFont(new Font("Serif",Font.BOLD,11));
		exit.setFont(myInfo.getFont().deriveFont(16.0f));
		exit.setForeground(Color.BLACK);
		exit.setBackground(Color.LIGHT_GRAY);
		add(exit);
		
		idontknow = new JButton("추가 예정");
		idontknow.setBounds(540, 25, 130, 50);
		idontknow.setFont(new Font("Serif",Font.BOLD,11));
		idontknow.setFont(myInfo.getFont().deriveFont(16.0f));
		idontknow.setForeground(Color.BLACK);
		idontknow.setBackground(Color.LIGHT_GRAY);
		add(idontknow);
		
		logo = new ImageIcon("./Saves/logo.png");
		logoImg = logo.getImage();
		logoImg = logoImg.getScaledInstance(110,110,Image.SCALE_SMOOTH);
		logo = new ImageIcon(logoImg);
		
		emoLabel = new JLabel(logo);
		emoLabel.setBounds(400, 0, 100, 100);
		add(emoLabel);
	...
	void dateOpen(String date){
		try {
			 String postName = "./Saves/post/";
			 postName = postName.concat(date);
			 postName = postName.concat(".txt");
			 
			 todayPost = new File(postName);
			 todayPost.createNewFile();
		 	 BufferedReader tempR = new BufferedReader(new FileReader(postName));
		 	 BufferedWriter tempW = new BufferedWriter(new FileWriter(postName,true));
